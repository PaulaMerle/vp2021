    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        if(isset($_POST["user_data_submit"])){
			...
			if(isset($_POST["first_name_input"]) and !empty($_POST["first_name_input"])) {
				$firstname = test_input(filter_var($_POST["first_name_input"], FILTER_SANITIZE_STRING)); // test_input tuleb fnc_general.php'st
				if(empty($firstname)) {
			...
			if(isset($_POST["surname_input"]) and !empty($_POST["surname_input"])) {
				$surname = test_input(filter_var($_POST["surname_input"], FILTER_SANITIZE_STRING)); // test_input tuleb fnc_general.php'st
				if(empty($surname)) {
			...
			if(isset($_POST["gender_input"]) and !empty($_POST["gender_input"])) {
				$gender = filter_var($_POST["gender_input"], FILTER_VALIDATE_INT); // täisarvu valideerimine (1 v 2)
			...
			if(isset($_POST["birth_day_input"]) and !empty($_POST["birth_day_input"])) {
				$birth_day = filter_var($_POST["birth_day_input"], FILTER_VALIDATE_INT); // täisarvu valideerimine (1 v 2)
			...
			if(isset($_POST["birth_month_input"]) and !empty($_POST["birth_month_input"])) {
				$birth_month = filter_var($_POST["birth_month_input"], FILTER_VALIDATE_INT); // täisarvu valideerimine (1 v 2)
				if($birth_month < 1 and $birth_month > 12) {
					$birth_month_error = " Palun märgi sünnikuu!";
				}
			} else {
				$birth_month_error = " Palun märgi sünnikuu!";
			}
			...
			if(isset($_POST["birth_year_input"]) and !empty($_POST["birth_year_input"])) {
				$birth_year = filter_var($_POST["birth_year_input"], FILTER_VALIDATE_INT); // täisarvu valideerimine (1 v 2)
				if($birth_year < date("Y") - 110 and $birth_year > date("Y") - 13) {
					$birth_year_error = " Palun märgi sünniaasta!";
				}
			} else {
				$birth_year_error = " Palun märgi sünniaasta!";
			}
			...
			if(empty($birth_day_error) and empty($birth_month_error) and empty($birth_year_error)) {
				if(checkdate($birth_month, $birth_day, $birth_year)) {
					$temp_date = new DateTime($birth_year ."-" .$birth_month ."-" .$birth_day); // annan ette vormi
					$birth_date = $temp_date->format("Y-m-d"); // formatin ette antud vormi
				} else {
					$birth_date_error = "Valitud kuupäeva ei ole olemas!";
				}
			}
			
			// e-mail
			if(isset($_POST["email_input"]) and !empty($_POST["email_input"])) {
				$email = test_input(filter_var($_POST["email_input"], FILTER_VALIDATE_EMAIL)); // valideerib e-posti aadressi
				if(empty($email)) {
			...
			if(isset($_POST["password_input"]) and !empty($_POST["password_input"])) {
				// parooli ei ole vaja filtreerida, seda ei tohi muuta
				// kontrollin parooli pikkust, ei tohi olla < 8
...
<html lang="et">
  ...
  <body>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <input name="first_name_input" id="first_name_input" type="text" value="<?php echo $firstname; ?>"><span style="color:red;"><?php echo $firstname_error; ?></span><br>
      <label for="surname_input">Perekonnanimi:</label><br>
	  <input name="surname_input" id="surname_input" type="text" value="<?php echo $surname; ?>"><span style="color:red;"><?php echo $surname_error; ?></span>
	  <br>
	  ...
	  <span style="color:red;"><?php echo $birth_date_error ." " .$birth_day_error ." " .$birth_month_error ." " .$birth_year_error; ?></span>
	  
	  <br>
	  <label for="email_input">E-mail (kasutajatunnus):</label><br>
	  <input type="email" name="email_input" id="email_input" value="<?php echo $email; ?>"><span style="color:red;"><?php echo $email_error; ?></span><br>
	  <label for="password_input">Salasõna (min 8 tähemärki):</label><br>
	  <input name="password_input" id="password_input" type="password"><span style="color:red;"><?php echo $password_error; ?></span><br>
	  <label for="confirm_password_input">Korrake salasõna:</label><br>
	  <input name="confirm_password_input" id="confirm_password_input" type="password"><span style="color:red;"><?php echo $confirm_password_error; ?></span><br>
